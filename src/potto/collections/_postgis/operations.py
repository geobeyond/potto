...
The functions defined in this module always return instances of ``potto.schemas.collections.Collection``.
"""
...
async def collect_all_collections(
    ...
    if user is None:
        public_collections = await collection_queries.collect_all_public_collections(
            session,
            collection_type_filter=collection_type_filter,
        )
        return [col.to_potto() for col in public_collections]
    accessible_ids = await authorization_backend.get_accessible_collection_identifiers(
        user
    )
    accessible_collections = await collection_queries.collect_all_user_collections(
        session,
        user_id=user.id,
        accessible_identifiers=accessible_ids,
        collection_type_filter=collection_type_filter,
    )
    return [accessible_col.to_potto() for accessible_col in accessible_collections]
...
async def paginated_list_collections(
    ...
    *,
    ...
    if user is None:
        public_collections, count = await collection_queries.paginated_list_public_collections(
            session,
            page=page,
            page_size=page_size,
    ...
    acessible_collections, count = await collection_queries.paginated_list_user_collections(
        session,
        page=page,
        page_size=page_size,
        include_total=include_total,
    ...
    return [accessible_col.to_potto() for accessible_col in acessible_collections], count
...
async def get_collection(
    ...
    resource = await collection_queries.get_collection(session, collection_id)
    if resource is None:
        return None
...
async def update_collection(
    ...
    try:
        ...
        if db_collection is None:
            raise PottoCollectionNotFoundException(f"collection {collection.identifier!r} not found")
...
async def delete_collection(
    ...
    if not await authorization_backend.can_edit_collection(user, db_collection.to_potto()):
